...
export function normalizeClustersData(data: unknown): GameData {
  ...
  return {
    ...
    regions: regions.filter(isObject).map((region) => ({
      ...
      name_key: typeof region.name_key === "string" ? region.name_key : undefined,
      alias_name: typeof region.alias_name === "string" ? region.alias_name : undefined,
      ...
      clusters: Array.isArray(region.clusters)
        ? region.clusters.filter(isObject).map((cluster) => ({
            ...
            location: typeof cluster.location === "string" ? cluster.location : undefined,
            location_key:
              typeof cluster.location_key === "string" ? cluster.location_key : undefined,
